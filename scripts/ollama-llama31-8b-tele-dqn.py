...
def get_action_from_llama(observation, good_training_step, good_examples):
    ...
    if good_examples.size > 0:
        example_states = "\n".join([f"State: {ex[0]}, Action: {action_map.get(ex[1], ['IDLE'])[0]}, Reward: {ex[2]}" for ex in good_examples])
    ...
    Given the current state of the transporation environment with the following observations: {tran_obs}
...
def get_telecom_action_from_dqn(transport_action, rf_cnt, thz_cnt, good_examples, epsilon=0.1):
...
def select_action(observation, epsilon, good_training_step, top_5_similar_good_examples,top_5_similar_good_examples_tele):
    if np.random.rand() < epsilon:
        ...
        telecom_action_text = get_telecom_action_from_dqn(transport_action, rf_cnt, thz_cnt,top_5_similar_good_examples_tele, epsilon)
...
for episode in range(num_episodes):
    ...
    while not (done or truncated) and step <= 30:
        ...
        top_5_similar_good_examples_tele = get_top_5_similar_good_examples(tele_state, good_examples_tele)

        dual_action = select_action(observation, epsilon, good_training_step, top_5_similar_good_examples,top_5_similar_good_examples_tele)
        ...
        if not truncated and reward > 0:
            ...
            # print("good_examples_tran",good_examples,good_examples.shape)
            # print("new_example_trab",new_example,new_example.shape)
            ...
            # print("good_examples_tele",good_examples_tele)
            # print("new_example_tele",new_example_tele)
